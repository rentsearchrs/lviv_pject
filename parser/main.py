...
#async def get_successful_ads():
    # Use `selectinload` to load relationships asynchronously
#    result = await db.execute(
#        select(Apartment).options(selectinload(Apartment.files)).filter(Apartment.ad_status == "successful")
#    )
#    successful_ads = result.scalars().all()
...
async def update_apartment_status(apartment_id: int, new_status: str = Query(...)):
    async with SessionLocal() as db:
...
async def assign_order_to_realtor(order_id: int, realtor_id: int):
    async with SessionLocal() as db:
        try:
            stmt = select(Order).where(Order.id == order_id)
            result = await db.execute(stmt)
...
async def read_apartment(apartment_id: int):
    async with SessionLocal() as db:
...
async def apply_watermark_to_existing_image(apartment_id: int, image_id: int):
...
async def upload_images(apartment_id: int, files: List[UploadFile] = File(...)):
    async with SessionLocal() as db:
        uploaded_images = []
...
async def reorder_images(apartment_id: int, order_updates: List[ImageOrderUpdate]):
    async with SessionLocal() as db:
...
async def update_fix_fields(apartment_id: int, update_data: dict):
    async with SessionLocal() as db:
...
async def get_template_text( template_name: str = "telegram") -> str:
    async with SessionLocal() as db:
...
async def publish_to_channel(apartment_id: int, template_name: str = "telegram"):
...
async def create_user(user: RieltorCreate):
    async with SessionLocal() as db:
...
async def assign_apartment(apartment_id: int):
    async with SessionLocal() as db:
...
async def follow_up(apartment_id: int):
    async with SessionLocal() as db:
...
async def set_lease(apartment_id: int, lease_months: int):
    async with SessionLocal() as db:
...
async def get_agent_notifications(agent_id: int):
    async with SessionLocal() as db:
        today = datetime.now()
...
async def mark_apartment_contacted(apartment_id: int):
    async with SessionLocal() as db:
...
async def archive_apartment(apartment_id: int):
    async with SessionLocal() as db:
        stmt = select(Apartment).where(Apartment.id == apartment_id)
        result = await db.execute(stmt)
        apartment = result.scalar_one_or_none()
...
async def get_my_apartments(token: str = Depends(oauth2_scheme)):
    async with SessionLocal() as db:
        try:
...
async def create_team_leader(team_leader: RieltorCreate):
    async with SessionLocal() as db:
        try:
...
async def get_team_leader_realtor_stats(team_leader_id: int):
    async with SessionLocal() as db:
        try:
...
async def get_order_statistics(realtor_id: int):
    async with SessionLocal() as db:
        try:
...
async def automated_telegram_posting():
    """
    Fetch apartments and send ads to Telegram channels at regular intervals.
    Stops dynamically when TELEGRAM_POSTING_RUNNING is False.
    """
        ...
        try:
            while TELEGRAM_POSTING_RUNNING:
                ...
                if not apartments_by_channel:
                    logging.info("No apartments to process.")
                ...
                for channel, apartments in apartments_by_channel:
                    for apartment in apartments:
                        ...
                        try:
                            ...
                            logging.info(f"✅ Posted apartment {apartment.id}. Waiting 2 minutes...")
                            await asyncio.sleep(120)  # Wait 2 minutes between postings
                        except Exception as e:
...
async def get_realtor_info(token: str = Depends(oauth2_scheme)):
    """
...
async def add_trap_word(data: WordModel):
    async with SessionLocal() as db:
...
async def add_stop_word(data: WordModel):
    async with SessionLocal() as db:
...
async def remove_trap_word(word: str):
    async with SessionLocal() as db:
...
async def remove_stop_word(word: str):
    async with SessionLocal() as db:
...
async def get_verification_ads():
...
async def verify_ad(apartment_id: int, decision: str):
    async with SessionLocal() as db:
